...
# ###################################
lll = int(sys.argv[-1])
data = torch.load("datasets/freebase/freebase-c.pt")
...
# output path
out_dir = "./out/freebase-c/2l2l2l-add-{}/".format(lll)
...
def train(epoch):
    ...
    # z = model.aa(torch.cat((z, z1, model.aa_embeddings), dim=1), data.aa_edge_idx, edge_weight=data.aa_edge_weight)
    z = model.aa(
