...
train = torch.from_numpy(pd.read_csv('datasets-auta/train_test_split/label.dat.train_{}'.format(lll), sep='\t', header=None).to_numpy().T)
test = torch.from_numpy(pd.read_csv('datasets-auta/train_test_split/label.dat.test_{}'.format(lll), sep='\t', header=None).to_numpy().T)
