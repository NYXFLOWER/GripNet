...
class homoGraph(Module):
    def __init__(
        ...
        if multi_relational:
            ...
            after_relu = [False if i == 0 else True for i in range(len(nhid_list) - 1)]
            self.conv_list = torch.nn.ModuleList(
                [
                    myRGCN(
                        nhid_list[i], nhid_list[i + 1], n_rela, n_base, after_relu[i]
                    )
...
class interGraph(Module):
    def __init__(
        ...
        if target_dim != target_feat_dim:
            self.target_feat_down = torch.nn.Parameter(
                torch.Tensor(self.target_feat_dim, target_dim)
            )
    ...
    def forward(self, x, inter_edge_index, edge_weight=None, if_relu=True, mod="cat"):
        ...
        if mod == "cat":
            ...
            if x.shape[1] == self.target_feat.shape[1]:
                ...
                x = (
                    x + F.relu(torch.matmul(self.target_feat, self.target_feat_down))
                ) / 2
