from gripnet.utils import EPS, sparse_id, typed_negative_sampling, auprc_auroc_ap
import numpy as np
...
model = Model(multiRelaInnerProductDecoder(embed_dim, data.n_edge_type)).to(device)
...
def train(epoch):
    ...
    neg_index = typed_negative_sampling(
        data.train_idx, data.n_node, data.train_range
    ).to(device)
    ...
    model.eval()
    neg_index = typed_negative_sampling(
        data.train_idx, data.n_drug, data.train_range[:-2]
    ).to(device)
...
test_neg_index = typed_negative_sampling(
    data.test_idx, data.n_drug, data.test_range[:-2]
).to(device)
