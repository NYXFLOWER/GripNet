...
print()
print("========================================================")
print("run: {} epochs === Aminer === {}".format(int(sys.argv[1]), "GripNet"))
print("========================================================")
...
# ###################################
lll = int(sys.argv[-1])
data = torch.load("datasets/aminer/aminer.pt")
train_set = torch.from_numpy(
    pd.read_csv(
        "datasets/aminer/train_test_split/label.dat.train_{}".format(lll),
        sep="\t",
        header=None,
    )
    .to_numpy()
...
test_set = torch.from_numpy(
    pd.read_csv(
        "datasets/aminer/train_test_split/label.dat.test_{}".format(lll),
        sep="\t",
        header=None,
    )
    .to_numpy()
...
# output path`
out_dir = "./out/aminer/auta-2l1l-u-{}/".format(lll)
