...
for epoch in range(1, args.epoch+1):
    ...
    line = 'Epoch{:3d} | Train loss: {:0.4f} | Train Micro: {:0.4f} | Train Macro: {:0.4f}\n' \
           '         | Test Micro: {:0.4f} | Test Macro: {:0.4f}       Time:{:0.2f}'.format(epoch, train_loss, train_micro, train_macro, test_micro, test_macro, (time.time() - time_begin))
    if epoch == 100:
        print(line)
    f.write(line + '\n')
f.close()
