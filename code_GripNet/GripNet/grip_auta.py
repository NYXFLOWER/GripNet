...
# ###################################
lll = int(sys.argv[-1])
data = torch.load('data/auta/auta.pt')
train = torch.from_numpy(pd.read_csv('data/auta/train_test/label.dat.train_{}'.format(lll), sep='\t', header=None).to_numpy().T)
test = torch.from_numpy(pd.read_csv('data/auta/train_test/label.dat.test_{}'.format(lll), sep='\t', header=None).to_numpy().T)
